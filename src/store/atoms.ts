...
export const activeHomeworkCountAtom = atom((get) => {
  const now = new Date()
  return get(homeworkListAtom).filter(hw => new Date(hw.dueDate) > now).length
})

// Lesson Progress Atoms
/**
 * Atom storing the lesson progress for the current class.
 */
export const lessonProgressAtom = atomWithMMKV<LessonProgress | null>(
  'lessonProgress',
  null,
)
