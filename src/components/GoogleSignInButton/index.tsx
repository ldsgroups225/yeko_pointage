import type { GoogleSignInButtonProps } from './type'
 ...
 * Based on official Google brand colors
 */
...
const GoogleSignInButton: React.FC<GoogleSignInButtonProps> = ({
  ...
  return (
    <AnimatedPressable
      ...
      className={`
        flex-row items-center justify-center rounded-xl border border-gray-200 dark:border-gray-700
        bg-white dark:bg-gray-800 shadow-sm
        ${sizeClasses[size]}
        ${(disabled || isLoadingState) ? 'opacity-60' : ''}
      `}
        ...
        : (
            <>
              <View className="w-5 h-5 items-center justify-center mr-2">
                <GoogleIcon size={iconSizes[size]} />
              </View>
              <Text
                testID={`${testID}-text`}
                style={textStyle}
                className={`
                  ...
                  ${(disabled || isLoadingState) ? 'text-gray-400 dark:text-gray-500' : ''}
                `}
              >
                {buttonText}
              </Text>
